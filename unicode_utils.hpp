...
std::string utf8_to_escape_unicode(const std::string& input) {
    ...
    while (pos < len) {
        ...
        if (byte <= 0x7F) {
            ...
            uint32_t code_point = ((byte & 0x1F) << 6) | 
                                 (static_cast<uint8_t>(input[pos + 1]) & 0x3F);
            
            result << "\\u" << std::hex << std::setw(4) << std::setfill('0') << code_point;
            pos += 2;
        } else if ((byte & 0xF0) == 0xE0) {
            // 3字节UTF-8
            if (pos + 2 >= len) break;
            
            uint32_t code_point = ((byte & 0x0F) << 12) |
            ...
            result << "\\u" << std::hex << std::setw(4) << std::setfill('0') << high_surrogate;
            result << "\\u" << std::hex << std::setw(4) << std::setfill('0') << low_surrogate;
            pos += 4;
        } else {
            // 无效的UTF-8序列
            pos++;
